...
def register_error_handlers(app):
    ...
    app.errorhandler(db.InvalidArgumentError)(partial(
        simple_error_guard,
        text='无效值.',
        status_code=400
    ))
    app.errorhandler(db.InvalidEmailError)(partial(
        simple_error_guard,
        text='无效邮箱.',
        status_code=400
    ))
    app.errorhandler(db.UniqueConstraintError)(partial(
        simple_error_guard,
        text='重复值错误.',
        status_code=400
    ))
    app.errorhandler(db.DeleteEmailInUseError)(partial(
        simple_error_guard,
        text='请退订相关订阅后再删除该邮箱.',
        status_code=400
    ))
    app.errorhandler(BusyError)(partial(
        simple_error_guard,
        text='更新姬反应不过来了... 请稍后重新查询 >_<',
        status_code=500
    ))
...
def general_error_guard(e):
    ...
    template = '出错了. 错误编号 %s . 您可以提交该编号给 %s , 协助改进torabot.'
